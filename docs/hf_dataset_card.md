...
| `docs/pai_indicators.csv` | the indicator framework behind each theme score: one row per version, theme and indicator (516 PAI 1.0 rows, 150 PAI 2.0), with numerator, denominator and kind (ratio, number or yes/no check), fetched from the portal's indicator browser |
